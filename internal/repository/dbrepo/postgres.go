...
// GetUserByID returns a user found by ID
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	r.processed, rm.id, rm.room_name from reservations r left join rooms rm on (r.room_id = rm.id) order by r.start_date asc`
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	var reservations []models.Reservation
	query := `select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date, r.room_id, r.created_at, r.updated_at,
	rm.id, rm.room_name from reservations r left join rooms rm on (r.room_id = rm.id) where processed = 0 order by r.start_date asc`
...
// GetRoomById returns one reservation by id
...
// DeleteReservation deletes a reservation in database by ID
...
// UpdateProcessedForReservation updates a processed for a reservation by ID
